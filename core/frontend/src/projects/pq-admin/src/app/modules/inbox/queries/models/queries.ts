...
export const MAX_ATTACHMENT_LIMIT = 7;
export const MAX_QUERY_LIMIT = 30;

export const QUERYSTATUS: MasterFilter[] = [
  { id: 1, master_name: 'Drafted', master_code: 'drafted' },
  { id: 2, master_name: 'Draft Rejected', master_code: 'draft_rejected' },
...
export interface JobQueries {
  ...
  sub_query_approval_required: number;
}
...
export interface Query {
  ...
  last_sub_query: string;
  show_attachment_input: boolean;
  attachments: QueryAttachment[],
  resolved_on: string;
  response_attachments: QueryAttachment[];
  query_template_code?: string;
  show_response_attachment_input: boolean;
  ...
  existingQueries?: {title: string, query: string, query_code: string, posted_date: string}[];
  isCheckingQueryExists?: boolean;
}
...
export class QueryAttachment {
  ...
  static addAttachmentTitle(title: string, user_id: number) {
    return {
      link: "",
      user_id: user_id,
      title: title,
...
export class Query {
  ...
  static defaultQuery(queryIndex: number = 0): Query {
    ...
    return {
      ...
      show_response_attachment_input: false,
      query_template_code: "",
      sub_query_count: 0,
      sub_query_approval_required: 0,
      filters: {
...
export interface QueryReply {
  ...
  raised_by_id: number;
  raised_by_name: string;
  raised_by_type: "client" | "internal" | undefined;
  parent_query_id: number;
  response_type: string;
  response_type_description: string;
  response_value: string;
...
export interface DraftQueryRequest {
  query_id: number;
  status_id: number;
  query: string;
  title: string;
...
export class DraftQueryRequest {
  static defaultDraftQueryRequest() {
    return {
      query_id: 0,
      status_id: 0,
      user_id: 0,
...
export class DraftJobQueries {
  static defaultDraftJobQueries() {
    return {
      job_id: "",
      job_name: "",
      date: "",
...
export interface BulkApproveQueryRequest {
  query_id: number;
  status: number;
  query: string;
  title: string;
  ...
  user_name: string;
  reject_reason: string;
  edited_reason: string | undefined;
}

export class BulkApproveQueryRequest {
  static defaultBulkApproveQueryRequest() {
    return {
      query_id: 0,
      status: 0,
      user_id: 0,
