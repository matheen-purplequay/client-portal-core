...
import { ArrowBigUp, CheckCircle, ChevronDown, Circle, Delete } from 'lucide-react'
...
export default function SimpleDropDown({ title, items, dropdownStyle = "default", selectedValue, onValueChange, containerClass, buttonClass, width = "default" }: SimpleDropDownProps) {
		...
		<Menu as="div" className={containerClasses}>
			<div>
				...
				>
					...
					<div className={`${dropdownButtonStyles[dropdownStyle]} ${buttonClass} ${width === "full" ? "w-full" : "w-auto"}`}>
						<span className={`text-primary ${buttonClass}`}>{selectedValue}</span>
...
export function KVDropDown({ title, items, dropdownStyle = "default", selectedValue, onValueChange, containerClass, buttonClass, width = "default" }: KVDropdownProps) {
	...
	useEffect(() => {
		...
		if (items && items.length > 0) {
			if (searchValue) {
				filtered = items.filter((item) => item.key.toLowerCase().includes(searchValue.toLowerCase()));
				setFilteredItems(filtered);
			} else {
				filtered = items;
				setFilteredItems(filtered);
		...
		<Menu as="div" className={containerClasses}>
			<div>
				...
				>
					<div className="text-xs text-left font-semibold text-slate-700">{title}</div>
					<div className={`${dropdownButtonStyles[dropdownStyle]} ${buttonClass} ${width === "full" ? "w-full" : "w-auto"}`}>
						<span className={`text-primary ${buttonClass}`}>{items.find((item) => item.key === selectedValue)?.value}</span>
						<ChevronDown aria-hidden="true" className="-mr-1 size-5 text-gray-400" />
					</div>
				</MenuButton>
			</div>
...
export function MultiSelectDropdown<T extends Record<string, any>>({
	...
	const toggleItem = (item: T) => {
		...
		// If shift is pressed, unselect everything and only select on clicked item
		if (isShiftPressed) {
			updated = [item];
			setSelected(updated);
			onChange(updated);
	...
	return (
		<Popover className="relative inline-block text-left w-56">
			...
			>
				...
				<div className={dropdownButtonStyles[dropdownStyle]}>
					<span className={buttonClassName}>
						{selected.length <= 0 && <span className="truncate text-primary flex-1 text-left">None selected</span>}
						{selected.length == 1 && <span className="truncate text-primary flex-1 text-left">{selected[0][labelKey]}</span>}
						{selected.length > 1 && <span className="truncate text-primary flex-1 text-left">{selected.length}/{items.length} selected</span>}
					</span>
			...
			>
					...
					{filteredItems.map((item, index) => {
						...
						return (
							<div
								key={index}
								onClick={() => {
									if (selected.includes(item) && selected.length <= minimumSelectedItems) {
										return; // block deselect
									}
									toggleItem(item);
								}}
								className={`
				...
				{/* Footer actions */}
				<div>
					<div className="text-xs text-center px-2 py-1 font-medium flex items-center justify-center bg-yellow-100 text-yellow-800">
						<ArrowBigUp size={14} strokeWidth={1.5} className='fill-current' />
						SHIFT +  Click to select only one
					</div>
					<div className="flex items-center justify-between gap-2 px-3 py-2">
						...
						{showClearAll && (
							<Button
								...
								className="px-0 outline-0 text-xs font-semibold"
								onClick={() => {
									if (items.length > minimumSelectedItems && minimumSelectedItems > 0) {
										const keep = [items[0]]; // always keep the first item
										setSelected(keep);
										onChange(keep);
									} else {
										setSelected([]);
