...
interface TableRowDataProps extends React.TdHTMLAttributes<HTMLTableCellElement> {
...
export const TableRowData = ({ children, className, colSpan, verticalAlign, ...props }: TableRowDataProps) => {
    return (<td className={`px-4 py-2 text-sm ${className}`} valign={verticalAlign} colSpan={colSpan} {...props}>{children}</td>);
};
...
export const TableHead = ({ children, className }: TableHeadProps) => {
    return (
        <thead
            className={`
            px-4 py-2 ${className} text-sm font-semibold text-slate-700 sticky top-0 border-b border-slate-300 text-nowrap z-10
            ${className?.includes('bg-') ? '' : 'bg-slate-100/80 backdrop-blur-sm'}
...
export const TableHeadCell = ({ children, className, onClick }: TableHeadCellProps) => {
    return (<th className={`px-4 py-2 text-xs text-nowrap truncate ${className}`} onClick={onClick}>{children}</th>);
};
